...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    let url = `${BACKEND_URL}/api/expenses/${id}`;
    
    // Add other query parameters if they exist (excluding id)
    ...
    console.error('Expense API proxy error:', error.message);
    ...
    if (error.response) {
      ...
      return res.status(500).json({ 
        error: { 
          message: 'Internal server error during expense operation' 
        } 
      });
    }
  }
